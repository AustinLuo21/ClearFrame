...
function getAnchor(person) {
  const nose = person.pose?.keypoints.find(
    (point) => point.part === 'nose' && point.score > 0.3,
  )
...
function getOwnerMask(person, entry, othersNearby) {
  ...
  for (let pixel = 0; pixel < shiftedMask.length; pixel++) {
    shiftedMask[pixel] =
      shiftedMask[pixel] && person.data[pixel] ? 1 : 0
  }
...
function matchAllowedPeople(people, frame) {
    ...
    people.forEach((person, index) => {
      ...
      if (recentlySeen) {
        const distance = Math.hypot(
          anchor.x - entry.point.x,
          anchor.y - entry.point.y,
        )

        const score =
          match - Math.min(distance / width, 1) * 0.3

        if (match >= 0.45 && score >= 0.35) {
          candidates.push({ entry, index, score, match })
        }
      } else if (match >= 0.6) {
        candidates.push({
          entry,
  ...
  candidates.forEach((candidate) => {
    if (
      usedEntries.has(candidate.entry)
    ...
    if (candidate.match > 0.7) {
      candidate.entry.colors =
        candidate.entry.colors.map((amount, index) => {
          return (
            amount * 0.95
            + personColors[candidate.index][index] * 0.05
          )
        })
    }
  })
...
function makeRemovalMask(people, assignments) {
  ...
  const mask = maskContext.createImageData(
    maskCanvas.width,
    maskCanvas.height,
  )

  const selectedMask = selectedContext.createImageData(
    selectedMaskCanvas.width,
    selectedMaskCanvas.height,
  )
  ...
  people.forEach((person, index) => {
    ...
    const protectedOwner = entry
      ? getOwnerMask(person, entry, othersNearby)
      : null
  ...
  blurredContext.clearRect(
    0,
    0,
    blurredMaskCanvas.width,
    blurredMaskCanvas.height,
  )

  protectedContext.clearRect(
    0,
    0,
    protectedMaskCanvas.width,
    protectedMaskCanvas.height,
  )
  ...
  offsets.forEach(([x, y]) => {
    blurredContext.drawImage(maskCanvas, x, y)
  })
  ...
  const removal = blurredContext.getImageData(
    0,
    0,
    blurredMaskCanvas.width,
    blurredMaskCanvas.height,
  ...
  const protection = protectedContext.getImageData(
    0,
    0,
    protectedMaskCanvas.width,
    protectedMaskCanvas.height,
...
function getColorCorrection(liveFrame, people, removalMask) {
  ...
  return channels.map((channel) => {
    const bottom =
      channel.count * channel.xx - channel.x * channel.x

    let gain = bottom
      ? (
        channel.count * channel.xy
        - channel.x * channel.y
      ) / bottom
      : 1
    ...
    let offset = channel.count
      ? (channel.y - gain * channel.x) / channel.count
      : 0
...
function getLightMap(
  ...
  for (let row = 0; row < rows; row++) {
    for (let column = 0; column < columns; column++) {
      ...
      const startY = Math.floor(
        row * inputCanvas.height / rows,
      )

      const endY = Math.floor(
        (row + 1) * inputCanvas.height / rows,
      )
      ...
      for (let y = startY; y < endY; y += 6) {
        for (let x = startX; x < endX; x += 6) {
          ...
          if (
            people.some((person) => person.data[pixel])
          ) continue

          if (
            removalMask.data[pixel * 4 + 3] > 8
          ) continue

          const color = pixel * 4

          red +=
            liveFrame.data[color]
            - (
              savedBackground.data[color]
              * correction[0].gain
              + correction[0].offset
            )

          green +=
            liveFrame.data[color + 1]
            - (
              savedBackground.data[color + 1]
              * correction[1].gain
              + correction[1].offset
            )

          blue +=
            liveFrame.data[color + 2]
            - (
              savedBackground.data[color + 2]
              * correction[2].gain
              + correction[2].offset
            )
  ...
  const lightMap = {
    columns,
    rows,
    cells: cells.map((cell) => cell || fallback),
  }

  if (lastLightMap) {
    lightMap.cells = lightMap.cells.map(
      (cell, index) => {
        ...
        return {
          ...
          green:
            previous.green * 0.8 + cell.green * 0.2,
          blue:
            previous.blue * 0.8 + cell.blue * 0.2,
        }
      },
    )
  }
...
function updateBackground(
  ...
  const covered = new Uint8Array(
    width * inputCanvas.height,
  )

  people.forEach((person) => {
    for (
      let pixel = 0;
      pixel < person.data.length;
      pixel++
    ) {
      if (person.data[pixel]) {
        covered[pixel] = 1
      }
    }
  })
  ...
  for (
    ...
    if (
      ...
      savedBackground.data[color] =
        savedBackground.data[color] * 0.997
        + targetRed * 0.003

      savedBackground.data[color + 1] =
        savedBackground.data[color + 1] * 0.997
        + targetGreen * 0.003

      savedBackground.data[color + 2] =
        savedBackground.data[color + 2] * 0.997
        + targetBlue * 0.003
    ...
    const matchesCandidate =
      ...
      && Math.abs(
        liveFrame.data[color + 1]
        - backgroundCandidates[candidate + 1],
      ) < 10
      && Math.abs(
        liveFrame.data[color + 2]
        - backgroundCandidates[candidate + 2],
      ) < 10

    if (matchesCandidate) {
      backgroundCandidateAge[pixel] = Math.min(
        255,
    ...
    if (backgroundCandidateAge[pixel] >= 5) {
      savedBackground.data[color] =
        savedBackground.data[color] * 0.65
        + targetRed * 0.35

      savedBackground.data[color + 1] =
        savedBackground.data[color + 1] * 0.65
        + targetGreen * 0.35

      savedBackground.data[color + 2] =
        savedBackground.data[color + 2] * 0.65
        + targetBlue * 0.35
    }
  }
}
...
function getLocalLight(x, y, lightMap) {
  const columnPosition =
    x / (width - 1) * (lightMap.columns - 1)

  const rowPosition =
    y / (inputCanvas.height - 1)
    * (lightMap.rows - 1)
  ...
  const topLeft =
    lightMap.cells[top * lightMap.columns + left]

  const topRight =
    lightMap.cells[top * lightMap.columns + right]

  const bottomLeft =
    lightMap.cells[bottom * lightMap.columns + left]

  const bottomRight =
    lightMap.cells[bottom * lightMap.columns + right]

  function blend(name) {
    const topValue =
      topLeft[name] * (1 - horizontalAmount)
      + topRight[name] * horizontalAmount

    const bottomValue =
      bottomLeft[name] * (1 - horizontalAmount)
      + bottomRight[name] * horizontalAmount

    return (
      topValue * (1 - verticalAmount)
      + bottomValue * verticalAmount
    )
  }
  ...
  return {
    red: Math.max(
      -40,
      Math.min(40, blend('red')),
    ),
    green: Math.max(
      -40,
      Math.min(40, blend('green')),
    ),
    blue: Math.max(
      -40,
      Math.min(40, blend('blue')),
    ),
  }
}
...
async function processFrame(generation) {
  ...
  try {
    ...
    const people =
      await model.segmentMultiPerson(
        inputCanvas,
        {
    ...
    const result =
      new ImageData(
        new Uint8ClampedArray(liveFrame.data),
        liveFrame.width,
        liveFrame.height,
      )

    const assignments =
      matchAllowedPeople(people, liveFrame)

    const processedRevision = sceneRevision

    // CHANGED:
    // Removal now works even when no people are allowed.
    if (savedBackground) {
      const masks =
        makeRemovalMask(people, assignments)

      const mask = masks.removal

      for (
        let pixel = 0;
        pixel < mask.width * mask.height;
        pixel++
      ) {
        const alpha = pixel * 4 + 3
      ...
      for (
        ...
        result.data[color] =
          liveFrame.data[color] * (1 - amount)
          + backgroundRed * amount

        result.data[color + 1] =
          liveFrame.data[color + 1]
          * (1 - amount)
          + backgroundGreen * amount

        result.data[color + 2] =
          liveFrame.data[color + 2]
          * (1 - amount)
          + backgroundBlue * amount
      }
    ...
    if (consecutiveProcessingErrors) {
      statusText.textContent =
        'Camera processing recovered'
    }
    ...
    if (
      consecutiveProcessingErrors >= 5
    ) {
      releaseCamera()

      statusText.textContent =
        'Camera processing stopped after repeated errors. Start the camera to retry.'
    } else {
      statusText.textContent =
        'Camera processing was interrupted. Retrying...'
...
async function startCamera() {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'The camera stopped while person detection was loading. Start it again to retry.',
      )
    }
    ...
    stream
      ...
      ?.addEventListener(
        ...
        () => {
          ...
          statusText.textContent =
            'Camera stopped. Start it again to continue.'
        },
      )
...
async function captureBackground() {
  ...
  statusText.textContent =
    'Capturing background. Keep the room empty...'
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'The camera is not ready. Start it again and retry.',
      )
    }
    ...
    for (
      ...
      if (
        ...
        throw new Error(
          'Background capture was cancelled',
        )
      }
    ...
    statusText.textContent =
      'Background saved. Step back in. With nobody selected, everyone will be removed.'
  } catch (error) {
    ...
    if (operationIsCurrent) {
      statusText.textContent =
        error.message
        || 'Could not capture the background'
    }
  } finally {
    if (
      operationId
...
function selectPerson(event) {
  ...
  if (personIndex === -1) {
    statusText.textContent =
      'No person found there. Try clicking their body.'

    return
  }

  const existing =
    currentAssignments[personIndex]

  if (existing) {
    allowedPeople =
      allowedPeople.filter(
        (entry) =>
          entry !== existing,
      )
    ...
    statusText.textContent =
      allowedPeople.length
        ? `Removed them. ${allowedPeople.length} allowed on camera`
        : 'Nobody selected — removing everyone.'
  ...
  const frame =
    lastCommittedFrame.raw

  const entry = {
    colors:
      getColors(person, frame),
    ...
    ownerMask:
      new Uint8Array(person.data),

    ownerAnchor: anchor,

    ownerSize:
      countMask(person.data),
  }
  ...
  statusText.textContent =
    `${allowedPeople.length} allowed on camera. Everyone else is removed`
}
...
function reset() {
  ...
  statusText.textContent =
    'Capture a new empty background'
}

function createMirroredCanvas(
  imageData,
) {
  const source =
...
async function checkFrame() {
  ...
  if (
    ...
    checkResult.textContent =
      'Capture a background and wait for the next frame.'
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'The frame images are too large to upload. Try again with a less detailed scene.',
      )
    }
    ...
    try {
      ...
      throw new Error(
        `AI check returned an invalid response (${response.status})`,
      )
    }
    ...
    if (
      ...
      throw new Error(
        'AI check returned an empty response',
      )
    }

    if (
      operationId === frameCheckId
      && sceneRevision
        === snapshot.revision
    ) {
      checkResult.textContent =
